...
export default Vue.component(`plugin-${PLUGIN_NAME}-recommendation`, {
  ...
  data: function (): RecommendationData {
    ...
    return {
      count: null, outOf: null, persist, query: null
    };
  },
  mounted() {
  ...
  methods: {
          ...
          try {
            ...
            if (json.isrcs.length > 0) {
              const response = await app.mk.api.v3.music(
                `/v1/catalog/${app.mk.storefrontCountryCode}/songs?filter[isrc]=${json.isrcs.join(", ")}`
              );
    ...
    visibilityChanged(visible: boolean, entry: any) {
      ...
      if (!this.cached[id] && visible) {
        app.mk.api.v3.music(`/v1/catalog/us/songs/${id}`)
          .then((data: any) => {
